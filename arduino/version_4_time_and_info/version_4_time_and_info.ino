...
void loop() {
  if (Serial_available() > 0) {
    ...
    if (strncmp(command, "AT+DELAY=", 9) == 0) {
      ...
      Serial_print_s(equipment);
      Serial_print_s(partNumber);
      Serial_print_s(serialNumber);
      Serial_print_s(pcbVersion);
      Serial_print_s(firmwareversion);
      Serial_print_s(manufacturer);
    }
    
    else {
      Serial_print_s("Invalid AT command,");
    }
  }
